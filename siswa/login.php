...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        $stmt = $db->prepare("SELECT * FROM users WHERE LOWER(username) = LOWER(?) AND password = ? AND role = 'siswa'");
        ...
        if ($user) {
            ...
            if ($stmt->fetch()) {
                $error = "Password salah atau akun bukan siswa. Cek kembali!";
            } else {
                $error = "Username '$username' tidak ditemukan. Daftar dulu!";
            }
        }
...
<head>
    ...
    <title>Login Siswa - SMK Colomadu</title>
